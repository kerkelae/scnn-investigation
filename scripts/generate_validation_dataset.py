...
odfs_sh = (
    ...
    @ odfs_sh[np.random.choice(len(odfs_sh), N_VAL)].unsqueeze(-1)
).squeeze(-1)
...
signals = torch.zeros(N_VAL, 120)

for i in range(0, N_VAL, BATCH_SIZE):
    print(f"{int(i / N_VAL * 100)}%", end="\r")
...
torch.save(signals, "../validation_signals.pt")
torch.save(targets, "../validation_targets.pt")
